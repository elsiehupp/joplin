...
class StyleContext {
	constructor() {
		this.globalStyle.marginRight = this.globalStyle.margin;
		this.globalStyle.marginLeft = this.globalStyle.margin;
		this.globalStyle.marginTop = this.globalStyle.margin;
		this.globalStyle.marginBottom = this.globalStyle.margin;

		this.globalStyle.icon = {
			fontSize: 30,
		};

		this.globalStyle.lineInput = {
			fontFamily: this.globalStyle.fontFamily,
			maxHeight: 22,
			height: 22,
			paddingLeft: 5,
		};

		this.globalStyle.headerStyle = {
			fontFamily: this.globalStyle.fontFamily,
		};

		this.globalStyle.inputStyle = {
			border: '1px solid',
			height: 24,
			maxHeight: 24,
			paddingLeft: 5,
		...
		this.globalStyle.containerStyle = {
			overflow: 'auto',
			overflowY: 'auto',
		};

		this.globalStyle.buttonStyle = {
			// marginRight: 10,
			border: '1px solid',
			minHeight: 26,
			minWidth: 80,
	...
	public themeStyle(theme:any) {
		...
		if (this.themeCache_[cacheKey]) return this.themeCache_[cacheKey];
		...
		const fontSizes:any = {
			...
			textAreaLineHeight: Math.round(this.globalStyle.textAreaLineHeight * editorFontSize / 12),
		};
		...
		// relevant properties
		output = Object.assign({}, this.globalStyle, fontSizes, this.themes[theme]);
		output = this.addMissingProperties(output);
		output = this.addExtraStyles(output);

		this.themeCache_[cacheKey] = output;
	...
	public buildStyle(cacheKey:any, themeId:string, callback:Function) {
		...
		if (this.cachedStyles_.themeId !== themeId) {
			this.cachedStyles_.themeId = themeId;
			this.cachedStyles_.styles = {};
		}

		if (this.cachedStyles_.styles[cacheKey]) return this.cachedStyles_.styles[cacheKey].style;

		const s = callback(themeStyle(themeId));

		this.cachedStyles_.styles[cacheKey] = {
			style: s,
			timestamp: Date.now(),
		};

		return this.cachedStyles_.styles[cacheKey].style;
	}
...
function themeStyle(theme:any) {
	return styleContext.themeStyle(theme);
}
